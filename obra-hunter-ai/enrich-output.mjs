...
for (const lead of leads) {
  ...
  if (postHits.length || heavyHits.length) {
    rejectedByPost.push({ ...lead, exclusion_reason: postHits.length ? "post_work_not_preventive" : "heavy_homologation_low_win" });
    continue;
  }
...
if (process.env.GITHUB_ENV) await writeFile(process.env.GITHUB_ENV, `EMAIL_READY=${commercialReady.length > 0 ? "true" : "false"}\n`, { flag: "a" });
...
function isCommercialReady(lead) {
  ...
  return [lead.telefone_alta_probabilidade, lead.email_alta_probabilidade, lead.formulario_ou_pagina_contato].some((item) => item && item !== "not_found");
}
...
function toSummary({ metadata, status, leads, researchBacklog }) {
  const lines = [`# Obra Hunter AI - ${metadata.run_id}`, "", `Status: ${status.state}`, `Modo: ${metadata.mode}`, `Gerado em: ${metadata.generated_at}`, "", "## Contatos", "", `- Leads prontos para comercial: ${leads.length}`, `- Sinais em pesquisa/backlog: ${researchBacklog.length}`, `- Com telefone: ${status.enrichment.contacts_with_phone}`, `- Com e-mail: ${status.enrichment.contacts_with_email}`, "", "## Top leads", ""];
  for (const lead of leads.slice(0, 10)) lines.push(`- ${lead.score} | ${lead.prioridade} | ${lead.company_name} | ${lead.city} | ${lead.chance_concorrencia} | contato: ${lead.contato_prioritario} | tel: ${lead.telefone_alta_probabilidade} | email: ${lead.email_alta_probabilidade}`);
...
function unknownContact() { return { contato_prioritario: "expansao, engenharia, facilities, obras ou compras", tipo_contato: "precisa qualificar contato direto", telefone_alta_probabilidade: "not_found", email_alta_probabilidade: "not_found", formulario_ou_pagina_contato: "not_found", linkedin_empresa: "not_found", linkedin_decisor: "not_found", fonte_contato_url: "not_found", qualidade_contato: "baixa", comentario_contato: "Automacao ainda nao encontrou telefone/e-mail publico confiavel; validar contato antes de abordagem fria." }; }
function chanceFor({ predictive, started, winnable, signalType }) { if (predictive.length && winnable.length) return "alta"; if (winnable.length) return "media_alta"; if (started.length) return "media"; if (["predictive_rollout", "commercial_expansion", "building_systems_upgrade", "corporate_fitout", "construction_hiring_signal"].includes(signalType)) return "media_alta"; return "media"; }
function priorityFor(chance, contactQuality) { if (chance === "alta" && /^alta/.test(contactQuality)) return "P0"; if (["alta", "media_alta"].includes(chance)) return "P1"; return "P2"; }
function filterReason({ predictive, started, winnable, signalType }) { const parts = []; if (predictive.length) parts.push(`janela preditiva: ${predictive.slice(0, 3).join(", ")}`); if (started.length) parts.push(`obra iniciada: ${started.slice(0, 3).join(", ")}`); if (winnable.length) parts.push(`concorrencia acessivel: ${winnable.slice(0, 3).join(", ")}`); if (!parts.length) parts.push(`sinal privado ${signalType}; precisa qualificar se concorrencia esta aberta`); return parts.join(" | "); }
function ticketEstimate(signalType) { if (["industrial_expansion", "warehouse_or_logistics"].includes(signalType)) return "R$1M-R$10M em pacote parcial"; if (["commercial_expansion", "predictive_rollout"].includes(signalType)) return "R$300k-R$2M por unidade ou pacote"; if (["corporate_fitout", "building_systems_upgrade"].includes(signalType)) return "R$300k-R$1.5M"; return "R$300k-R$10M, sujeito a escopo"; }
function personaFor(signalType, started) { if (started.length) return "empreiteiro principal, compras, engenharia ou gerente de obra"; if (["commercial_expansion", "predictive_rollout"].includes(signalType)) return "expansao, novos negocios, franquias, implantacao ou operacoes"; if (["industrial_expansion", "warehouse_or_logistics"].includes(signalType)) return "engenharia, expansao, manutencao industrial ou compras"; if (["corporate_fitout", "building_systems_upgrade"].includes(signalType)) return "facilities, engenharia, obras ou compras"; return "expansao, engenharia, facilities ou compras"; }
function compareSalesPriority(a, b) { const priorities = { P0: 0, P1: 1, P2: 2, P3: 3 }; const chances = { alta: 0, media_alta: 1, media: 2, baixa: 3 }; return (priorities[a.prioridade] ?? 9) - (priorities[b.prioridade] ?? 9) || (chances[a.chance_concorrencia] ?? 9) - (chances[b.chance_concorrencia] ?? 9) || b.score - a.score || String(a.company_name).localeCompare(String(b.company_name)); }
function leadText(lead) { return [lead.company_name, lead.title, lead.rationale, lead.recommended_approach, lead.city, lead.source_name, lead.raw?.text, lead.raw?.title, lead.raw?.company_name, lead.raw?.raw_query].filter(Boolean).join(" "); }
function hits(text, keywords) { const normalized = norm(text); return keywords.filter((keyword) => contains(normalized, keyword)); }
function contains(text, keyword) { const normalized = norm(keyword).replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s+"); return new RegExp(`(^|[^a-z0-9])${normalized}($|[^a-z0-9])`).test(text); }
...
function c(aliases, contato_prioritario, tipo_contato, telefone_alta_probabilidade, email_alta_probabilidade, formulario_ou_pagina_contato, linkedin_empresa, linkedin_decisor, fonte_contato_url, qualidade_contato, comentario_contato) { return { aliases, contato_prioritario, tipo_contato, telefone_alta_probabilidade, email_alta_probabilidade, formulario_ou_pagina_contato, linkedin_empresa, linkedin_decisor, fonte_contato_url, qualidade_contato, comentario_contato }; }
function parseArgs(argv) { const out = {}; for (const arg of argv) { if (!arg.startsWith("--")) continue; const [key, value = "true"] = arg.slice(2).split("="); out[key.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase())] = value; } return out; }
